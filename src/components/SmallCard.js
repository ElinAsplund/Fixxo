    ...
    return (
        <div className="small-card" id={item.idCardNumber}>
            <div className="placeholder-area">
                ...
                <NavLink to={`/products/${item.productName.replace(/ /gi, "_").toLowerCase()}`}end>
                    <button className="btn-bg-theme btn-quick-view">QUICK VIEW</button>
                </NavLink>
            </div>
            <h2>{item.category}</h2>
            <NavLink to={`/products/${item.productName.replace(/ /gi, "_").toLowerCase()}`} className="product-name" end>{item.productName}</NavLink>
